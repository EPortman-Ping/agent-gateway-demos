...
The user authenticates via the Chat UI (PingOne PKCE) and sends a message to the Agent Bridge. The bridge validates the user's token and stores it raw in the ADK session state, then invokes Agent Runtime. The agent reads the user token from session state and performs an RFC 8693 exchange to produce a delegated token (`sub=user, act.client_id=agent`), which it attaches to each outbound MCP request. Agent Runtime routes those requests through the Agent Gateway, which calls the extension service. The extension service validates the delegated token, calls PingOne Authorize with the request context, and on PingOne Authorize approval performs a second RFC 8693 exchange to produce a tool-scoped token before the request reaches the target MCP server.
