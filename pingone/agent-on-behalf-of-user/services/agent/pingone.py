...
For step-up: if the bridge stored an elevated_token in state after MFA approval,
that token is used directly (it already has the high-value scope).
"""
...
def mcp_headers(ctx) -> dict[str, str]:
    ...
    try:
        ...
        logging.warning("Failed to get delegated token: %s", exc)
