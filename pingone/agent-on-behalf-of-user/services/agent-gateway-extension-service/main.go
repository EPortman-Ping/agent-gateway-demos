...
// that token as the Authorization header. Requests above STEP_UP_THRESHOLD_CENTS
// are evaluated against PingOne Authorize before the token is injected.
...
func main() {
	...
	stepUpThreshold := 100000
	if v := os.Getenv("STEP_UP_THRESHOLD_CENTS"); v != "" {
