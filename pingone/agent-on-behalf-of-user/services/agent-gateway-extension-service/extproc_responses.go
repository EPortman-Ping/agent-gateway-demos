...
// stepUpRequired returns a 401 with a structured step_up_required body. The
// agent bridge detects this, triggers a PingOne MFA push for the user, and
// re-submits the request with an elevated token (amr=mfa) after approval.
func stepUpRequired(amountCents, thresholdCents int) *extprocv3.ProcessingResponse {
	body, _ := json.Marshal(map[string]any{
		"error":            "step_up_required",
		"error_description": fmt.Sprintf("transaction amount $%.2f exceeds the $%.2f step-up threshold — MFA required", float64(amountCents)/100, float64(thresholdCents)/100),
			...
			ImmediateResponse: &extprocv3.ImmediateResponse{
				...
				Headers: &extprocv3.HeaderMutation{
					SetHeaders: []*corev3.HeaderValueOption{
						...
						{Header: &corev3.HeaderValue{Key: "www-authenticate", RawValue: []byte(`Bearer error="insufficient_user_authentication",acr_values="mfa"`)}},
					},
				},
				Body:    body,
