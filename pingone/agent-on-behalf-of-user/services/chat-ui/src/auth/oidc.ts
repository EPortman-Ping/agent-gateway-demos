...
const STEP_UP_SCOPES = (import.meta.env.VITE_STEP_UP_SCOPES as string) || `${SCOPES} stripe_mcp:high_value`;
...
const STORAGE_KEYS = {
  ...
  pendingMessage: 'oidc_pending_message',
  stepUpReturn: 'oidc_step_up_return',
} as const;
...
/**
 * Redirect through PKCE requesting elevated scopes (includes stripe_mcp:high_value).
 * PingOne will enforce MFA as part of the authorization flow.
 * The pending message is saved so the UI can auto-submit after callback.
 */
export async function stepUpLogin(pendingMessage: string): Promise<void> {
  sessionStorage.setItem(STORAGE_KEYS.pendingMessage, pendingMessage);
  sessionStorage.setItem(STORAGE_KEYS.stepUpReturn, '1');
...
/**
 * If we just returned from a step-up PKCE redirect, consume and return the
 * pending message. Returns null if this was a normal login callback.
 */
export function consumePendingStepUp(): string | null {
  const isStepUp = sessionStorage.getItem(STORAGE_KEYS.stepUpReturn);
  const message = sessionStorage.getItem(STORAGE_KEYS.pendingMessage);
  if (!isStepUp || !message) return null;
  sessionStorage.removeItem(STORAGE_KEYS.stepUpReturn);
  sessionStorage.removeItem(STORAGE_KEYS.pendingMessage);
  return message;
}
