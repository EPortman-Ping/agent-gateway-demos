...
import { logout, getAccessToken, getUserInfo, stepUpLogin, consumePendingStepUp } from '../auth/oidc';
...
export default function ChatScreen() {
  ...
  // After a step-up PKCE redirect, auto-submit the pending message with the elevated token.
  useEffect(() => {
    const pending = consumePendingStepUp();
    if (!pending) return;
    ...
    setLoading(true);
    invokeAgent(pending, token)
      .then((reply) => addMessage('agent', reply))
      .catch((err) => addMessage('error', err instanceof Error ? err.message : 'Unknown error'))
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.includes('step_up_required')) {
        // Redirect through PingOne PKCE with elevated scopes — PingOne enforces MFA.
        // The pending message is saved and auto-submitted after the callback.
        await stepUpLogin(input || text);
      } else {
