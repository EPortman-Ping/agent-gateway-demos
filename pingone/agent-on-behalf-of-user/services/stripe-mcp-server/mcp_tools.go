...
func createStripePaymentIntentTool() (mcp.Tool, server.ToolHandlerFunc) {
	...
	handler := func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		// High-value purchases require the step-up scope (MFA-elevated token).
		if stepUpScope != "" {
			grantedScope, _ := ctx.Value(ctxKeyGrantedScope).(string)
			if !tokenHasScope(grantedScope, stepUpScope) {
				totalPrice := req.GetFloat("total_price", 0)
				log.Printf("tool=create_stripe_payment_intent — scope %q required for %.2f but not present", stepUpScope, totalPrice)
				return mcp.NewToolResultError(fmt.Sprintf("step_up_required: scope %q is required for this transaction — MFA elevation needed", stepUpScope)), nil
			}
		}
