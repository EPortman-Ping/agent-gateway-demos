...
Support Agent's and Order Status Agent's own exchanges both target the same shared intermediate resource, the google cloud agent gateway — never the real final resource directly. Only the gateway extension's exchanges ver touch the real final resources.
...
**0. Raw browser token ----> Support Agent**
```json
{
  "sub": "<bob-user-id>",
  "client_id": "<chat-ui-client-id>",
  "aud": ["support-agent"],  # token only works on Support Agent
  "scope": "support-agent:invoke openid profile email",
  "act": "noActor",  # no delegation yet
  "may_act": {"sub": "<support-agent-client-id>"}  # only support agent can exchange this token
}
```

**1. Support Agent's token ----> Agent Gateway**
```json
{
  "sub": "<bob-user-id>",
  "client_id": "<support-agent-client-id>",
  "aud": ["ac-google-cloud-agent-gateway"],  # token only works on Agent Gateway
  "scope": "order-status:invoke",
  "act": {
    "sub": "<support-agent-client-id>", "act": "noActor"  # token delegagted by support agent
  },
  "may_act": {"sub": "<agent-gw-extension-client-id>"}  # only agent gateway extension can exchange this token
}
```

**2. Agent Gateway ---->  Order Status Agent**
```json
{
  "sub": "<bob-user-id>",
  "client_id": "<agent-gw-extension-client-id>",
  "aud": ["order-status-agent"],  # token only works on Order Status Agent
  "scope": "order-status:invoke",
  "act": {
    "sub": "<agent-gw-extension-client-id>", "act": {  # token delegagted by gateway service extension 
      "sub": "<support-agent-client-id>", "act": "noActor"}  # token delegagted by support agent
  },
  "may_act": {"sub": "<order-status-agent-client-id>"}  # only order status agent can exchange this token
}
```

**3. Order Status Agent ----> Agent Gateway**
```json
{
  "sub": "<bob-user-id>",
  ...
  "scope": "order:read",
  "act": {
    "sub": "<order-status-agent-client-id>", "act": {  # token delegagted by Order Status Agent
      "sub": "<agent-gw-extension-client-id>", "act": {  # token delegagted by gateway service extension 
        "sub": "<support-agent-client-id>", "act": "noActor"  # token delegagted by support agent
  }}},
  "may_act": {"sub": "<agent-gw-extension-client-id>"}  # only agent gateway extension can exchange this token
...
**4. Agent Gateway ---->  Order Status MCP Server**
```json
{
  "sub": "<bob-user-id>",
  "client_id": "<agent-gw-extension-client-id>",
  "aud": ["order-status-mcp-server"],  # token only works on Order Status MCP Server
  "scope": "order:read",
  "act": {
    "sub": "<agent-gw-extension-client-id>", "act": {  # token delegagted by gateway service extension 
      "sub": "<order-status-agent-client-id>", "act": {  # token delegagted by Order Status Agent
