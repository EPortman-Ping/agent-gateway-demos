...
- Scopes: `openid profile email support-agent:invoke` — grant `support-agent:invoke` from the `AC Support Agent` PingOne resource (see [support-agent's README](../support-agent/README.md#pingone-resource-setup-inbound-token-audience)). This scope is what makes PingOne audience the resulting login token to `support-agent` — no `audience`/`resource` parameter is ever sent by this app; PingOne derives `aud` from whichever resource owns the requested scope.
